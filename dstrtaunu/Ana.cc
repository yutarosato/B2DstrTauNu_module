...
namespace Belle{
		      ...
		      BelleTuple* dist, const bool fl_message )
  {
	...
	Particle* l_lep = ( info_Dtau1.cos() > info_Dtau2.cos() ?  &Dtau2.child(1) : &Dtau1.child(1) ); // lepton with low value of cos theta(B-D*l)
	...
	dist->column( "eecl",    calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, eecl_detail, neecl_detail, l_lep, NULL, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	...
	int rempi0 =   cnt_remain_pi0( Dtau1, Dtau2, pi0_list_test1 );
	...
	dist->column( "remg",    cnt_remain_gamma_energy(Dtau1,Dtau2,gam_list, 0.10, 0.05, 0.15 ) );
	dist->column( "remg05",  cnt_remain_gamma_energy(Dtau1,Dtau2,gam_list, 0.05, 0.05, 0.05 ) );
	dist->column( "remg10",  cnt_remain_gamma_energy(Dtau1,Dtau2,gam_list, 0.10, 0.10, 0.10 ) );
	dist->column( "remg15",  cnt_remain_gamma_energy(Dtau1,Dtau2,gam_list, 0.15, 0.15, 0.15 ) );

	// +++++++++++++++++++++++++++++++++++++++++++++
	// pi0 study
	dist->column( "rempi0_0", cnt_remain_pi0(Dtau1, Dtau2, pi0_list_test0) );
	dist->column( "rempi0_1", cnt_remain_pi0(Dtau1, Dtau2, pi0_list_test1) ); // default setting
	...
	// ++++++++++++++++++++++++++++++++
	// study extra pi0 samples
	Particle& Dtau_l = ( info_Dtau1.cos() > info_Dtau2.cos() ? Dtau2 : Dtau1 );
	Particle& Dtau_h = ( info_Dtau1.cos() < info_Dtau2.cos() ? Dtau2 : Dtau1 );
	Particle& D_l    = ( info_Dtau1.cos() > info_Dtau2.cos() ? D2    : D1    );
	Particle& D_h    = ( info_Dtau1.cos() < info_Dtau2.cos() ? D2    : D1    );

	Particle* extra_pi =  search_remain_pi0(Dtau1, Dtau2, pi0_list);
	dist->column( "eecl_exp", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi, "epi", dist );

	Particle* extra_pi0 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test0); // valid only for #extra_pi0==1
	dist->column( "eecl_ex0", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi0, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi0, "epi0", dist );
	
	Particle* extra_pi1 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test1); // valid only for #extra_pi0==1
	dist->column( "eecl_ex1", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi1, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi1, "epi1", dist );

	Particle* extra_pi2 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test2); // valid only for #extra_pi0==1
	dist->column( "eecl_ex2", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi2, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi2, "epi2", dist );

	Particle* extra_pi3 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test3); // valid only for #extra_pi0==1
	dist->column( "eecl_ex3", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi3, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi3, "epi3", dist );

	Particle* extra_pi4 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test4); // valid only for #extra_pi0==1
	dist->column( "eecl_ex4", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi4, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi4, "epi4", dist );

	Particle* extra_pi5 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test5); // valid only for #extra_pi0==1
	dist->column( "eecl_ex5", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi5, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi5, "epi5", dist );

	Particle* extra_pi6 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test6); // valid only for #extra_pi0==1
	dist->column( "eecl_ex6", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi6, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi6, "epi6", dist );

	Particle* extra_pi7 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test7); // valid only for #extra_pi0==1
	dist->column( "eecl_ex7", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi7, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi7, "epi7", dist );

	Particle* extra_pi8 =  search_remain_pi0(Dtau1, Dtau2, pi0_list_test8); // valid only for #extra_pi0==1
	dist->column( "eecl_ex8", calEclEnergyWithMatch2GoodGamma_mask(Dtau1, Dtau2, !true, NULL, NULL, l_lep, extra_pi8, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) );
	extra_pi0_study( Dtau_l, Dtau_h, D_l, D_h, extra_pi8, "epi8", dist );
	...
	dist->column( "d1p",     D1.p().vect().mag()        ); // modified @20150305
	dist->column( "d2p",     D2.p().vect().mag()        ); // modified @20150305
	...
	///*
	if( info_Dtau1.cos() > info_Dtau2.cos() ){
	  int hyp = ( info_Dtau2.rec_mode()==10 ? 0 : 1 ); // 0(e), 1(mu)
	  dist->column( "ltrkchi", lep2.mdstCharged().trk().mhyp(hyp).chisq() );
	  dist->column( "ltrkndf", lep2.mdstCharged().trk().mhyp(hyp).ndf()   );
	  dist->column( "ltrkcl",  chisq2Confi(lep2.mdstCharged().trk().mhyp(hyp).ndf(), lep2.mdstCharged().trk().mhyp(hyp).chisq()) );
	}else{
	  int hyp = ( info_Dtau1.rec_mode()==10 ? 0 : 1 ); // 0(e), 1(mu)
	  dist->column( "ltrkchi", lep1.mdstCharged().trk().mhyp(hyp).chisq() );
	  dist->column( "ltrkndf", lep1.mdstCharged().trk().mhyp(hyp).ndf()   );
	  dist->column( "ltrkcl",  chisq2Confi(lep1.mdstCharged().trk().mhyp(hyp).ndf(), lep1.mdstCharged().trk().mhyp(hyp).chisq()) );
	}
	//*/
	///*
	...
	// +++++++++++++++++++++++++ study recovery for low energy muon track ++++++++++++++++++++++++
	/*
	...
	dist->column( "mu1ecl",   -10 );
	dist->column( "mu1reg",   -10 );
	dist->column( "mu1org",   -10 );
	dist->column( "mu1th",    -10 );
	dist->column( "mu1e9e25", -10 );
	dist->column( "mu1ewid",  -10 );
	dist->column( "mu2ecl",   -10 );
	dist->column( "mu2reg",   -10 );
	dist->column( "mu2org",   -10 );
	dist->column( "mu2th",    -10 );
	dist->column( "mu2e9e25", -10 );
	dist->column( "mu2ewid",  -10 );
	int tmp_cnt1=0;
	int tmp_cnt2=0;
	...
	for( Mdst_ecl_Manager::iterator iecl = eclmgr.begin();
	  ...
	  if( iecl->match()==0 ) continue;
	  Mdst_ecl_aux& shower_aux(ecl_aux(Panther_ID(iecl->get_ID())));
	  const Mdst_trk& mug_trk = shower_aux.trk();
	  const Mdst_trk& mu1_trk = lep1.mdstCharged().trk();
	  const Mdst_trk& mu2_trk = lep2.mdstCharged().trk();
	  //std::cout << mug_trk.get_ID() << ": "
	  //<< mu1_trk.get_ID() << ": "
	  //<< mu2_trk.get_ID() << std::endl;

	  if( mug_trk.get_ID()==mu1_trk.get_ID() ) {
	    ...
	    int fl_threshold = checkEECLCluster( iecl->energy(), iecl->theta(), 0.10, 0.05, 0.15 ); // 1(forward), 2(barrel), 3(backward)   with  threshold
	    int fl_origin    = ( mc==0 ? 0 : checkEECLClusterOrigin( isim->hepevt(), fl_message ) );
	    dist->column( "mu1ecl",  iecl->energy() );
	    dist->column( "mu1reg",  fl_region      );
	    dist->column( "mu1org",  fl_origin      );
	    dist->column( "mu1th",   fl_threshold   );
	    float fe9oe25 = shower_aux.e9oe25();
	    float fwidth = shower_aux.width();
	    dist->column( "mu1e9e25",  fe9oe25 );
	    dist->column( "mu1ewid",   fwidth  );
	    tmp_cnt1++;
	  }

	  if( mug_trk.get_ID()==mu2_trk.get_ID() ) {
	    // check if cluster energy exceed the threshold
	    int fl_region    = checkEECLCluster( iecl->energy(), iecl->theta(),  0.0,  0.0,  0.0 ); // 1(forward), 2(barrel), 3(backward) without threshold
	    int fl_threshold = checkEECLCluster( iecl->energy(), iecl->theta(), 0.10, 0.05, 0.15 ); // 1(forward), 2(barrel), 3(backward)   with  threshold
	    int fl_origin    = ( mc==0 ? 0 : checkEECLClusterOrigin( isim->hepevt(), fl_message ) );
	    dist->column( "mu2ecl",  iecl->energy() );
	    dist->column( "mu2reg",  fl_region      );
	    dist->column( "mu2org",  fl_origin      );
	    dist->column( "mu2th",   fl_threshold   );
	    float fe9oe25 = shower_aux.e9oe25();
	    float fwidth = shower_aux.width();
	    dist->column( "mu2e9e25",  fe9oe25 );
	    dist->column( "mu2ewid",   fwidth  );
	    tmp_cnt2++;
	  }
	}
	*/
	...
	// ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++;
	// additional ks information
	dist->column( "ks1m",     -1 );
	dist->column( "ks1self",   1 );
	for( int i=0; i<D1.nChildren(); i++ ){
	  if( D1.child(i).lund()==Ks_LUND ){
	    UserInfo& info_ks = dynamic_cast<UserInfo&>( D1.child(i).userInfo() );
	    dist->column( "ks1m",     D1.child(i).mass() );
	    dist->column( "ks1self",  info_ks.self()     );
	  }
	}
	
	dist->column( "ks2m",     -1 );
	dist->column( "ks2self",   1 );
	
	for( int i=0; i<D2.nChildren(); i++ ){
	  if( D2.child(i).lund()==Ks_LUND ){
	    UserInfo& info_ks = dynamic_cast<UserInfo&>( D2.child(i).userInfo() );
	    dist->column( "ks2m",     D2.child(i).mass() );
	    dist->column( "ks2self",  info_ks.self()  );
	  }
	}
	...
	*/
	...
	dist->column( "eecl_ctrl", calEclEnergyWithMatch2GoodGamma(Dtau1, Dtau2, !true, NULL, NULL, veto_pi0, gen_b_decay_info[0][0], gen_b_decay_info[1][0] ) ); // * this function has been already charnged for other purpose (obsolete)
	...
	*/
	...
	dist->column( "ddst1_acc", gen_b_decay_info[0][25] ); dist->column( "ddst2_acc", gen_b_decay_info[1][25] ); // 201407034
	
	if( gen_b_decay_info[0][13]==3 ){ // 20150717
	  Gen_hepevt& d_from_dstst( genMgr( Panther_ID(gen_b_decay_info[0][7]) ) ); // D from D** decay
	  int nchild = d_from_dstst.daLast()==0 ? 0 : d_from_dstst.daLast() - d_from_dstst.daFirst() + 1;
	  dist->column( "ddst1_nch", nchild );
	}else{
	  dist->column( "ddst1_nch", -1 );
	}

	if( gen_b_decay_info[1][13]==3 ){ // 20150717
	  Gen_hepevt& d_from_dstst( genMgr( Panther_ID(gen_b_decay_info[1][7]) ) ); // D from D** decay
	  int nchild = d_from_dstst.daLast()==0 ? 0 : d_from_dstst.daLast() - d_from_dstst.daFirst() + 1;
	  dist->column( "ddst2_nch", nchild );
	}else{
	  dist->column( "ddst2_nch", -1 );
	}
	...
	calKinematics_np( gen_b_decay_info[0][0], gen_b_decay_info[0][7], gen_b_decay_info[0][2],
			  ...
			  cos_dau,
			  p_dau_tau, p_dau_w, p_dau_b, p_dau_lab );
	
	dist->column( "q2",        q2        );
	dist->column( "cos_d",     cos_d     );
	dist->column( "cos_w",     cos_w     );
	dist->column( "cos_t",     cos_t     );
	dist->column( "cos_dau",   cos_dau   );
	dist->column( "p_dau_tau", p_dau_tau );
	dist->column( "p_dau_w",   p_dau_w   );
	dist->column( "p_dau_b",   p_dau_b   );
	dist->column( "p_dau_lab", p_dau_lab );

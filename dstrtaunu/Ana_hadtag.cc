...
namespace Belle{
			     ...
			     BelleTuple* dist, const bool fl_message )
  {
	...
	// q2 calculation is added @ 20150421
	HepLorentzVector p_B( -b_4Vcm.px(), -b_4Vcm.py(), -b_4Vcm.pz(), eb );
	UserInfo& info_Dstr = dynamic_cast<UserInfo&>( Dtau.child(0).userInfo() );
	HepLorentzVector p_Dstr = info_Dstr.Vcm();
	HepLorentzVector p_q2 = p_B - p_Dstr;
	double q2rec = p_q2.mag()*fabs(p_q2.mag());
	dist->column( "q2rec", q2rec );

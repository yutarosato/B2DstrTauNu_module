...
namespace Belle{
  ...
  void DSTRTAUNU::hist_def()
  {
    ...
    std::stringstream sTmp_np;
    sTmp_np << " exprun evt_type "             // [   number   ]
	    << " blund dlund q2 "              // [ lund & q2  ]
	    << " cos_d cos_w cos_t cos_dau"    // [decay angle ]
	    << " p_dau_tau p_dau_w p_dau_b p_dau_lab p_dau_cm " // [tau's daugher momentum]
	    << " dec_tau dec_d "               // [ decay mode ]
	    << " dstp dstpcm "                 // [  momentum  ]
	    << " evis2 mmiss2 "
	    << " cosdll cosdlh cosdl1 cosdl2 " // [ fitting variables ]
      ;
    NP_dist = tm->ntuple( "np",  sTmp_np.str().c_str(), 37 );      

...
bool DSTRTAUNU::myCheckSame(const Particle& p, const Mdst_ecl& ecl)
{
  ...
  void DSTRTAUNU::calKinematics_np( int id_b, int id_d, int id_tau,
				    int dec_d, int dec_tau,
				    double& q2, double& cos_d, double& cos_w, double& cos_t,
				    double& cos_daughter_w,
				    double& p_daughter_tau, double& p_daughter_w, double& p_daughter_b, double& p_daughter_lab, double& p_daughter_cm
				    ){
    if( !id_tau ) return;
    Gen_hepevt_Manager& genMgr = Gen_hepevt_Manager::get_manager();
    Gen_hepevt& B  ( genMgr(Panther_ID(id_b  ) )); HepLorentzVector   B_4V(   B.PX(),   B.PY(),   B.PZ(),   B.E() );
    Gen_hepevt& D  ( genMgr(Panther_ID(id_d  ) )); HepLorentzVector   D_4V(   D.PX(),   D.PY(),   D.PZ(),   D.E() );
    Gen_hepevt& Tau( genMgr(Panther_ID(id_tau) )); HepLorentzVector Tau_4V( Tau.PX(), Tau.PY(), Tau.PZ(), Tau.E() );
    HepLorentzVector W_4V = B_4V - D_4V;
    ...
    // [cos-theta(W)]
    Tau_4V.boost( -(W_4V.boostVector()) ); // boost to W-rest frame
    B_4V.boost  ( -(W_4V.boostVector()) ); // boost to W-rest frame
    cos_w = cos( Tau_4V.vect().angle(B_4V.vect()) ); // [cos-theta(W)]
    Tau_4V.boost(  (W_4V.boostVector()) ); // boost back to lab. frame
    B_4V.boost  (  (W_4V.boostVector()) ); // boost back to lab. frame


    // [cos-theta(D)]
    if( 1 < dec_d && dec_d < 7 ){
      Gen_hepevt& Dau ( genMgr(Panther_ID(D.daFirst())) );
      HepLorentzVector Dau_4V( Dau.PX(), Dau.PY(), Dau.PZ(), Dau.E() );
      Dau_4V.boost( -(D_4V.boostVector()) ); // boost to D*-rest frame
      B_4V.boost  ( -(D_4V.boostVector()) ); // boost to D*-rest frame
      cos_d = cos( Dau_4V.vect().angle(B_4V.vect()) ); // [cos-theta(D)]
      Dau_4V.boost(  (D_4V.boostVector()) ); // boost back to lab. frame
      B_4V.boost  (  (D_4V.boostVector()) ); // boost back to lab. frame
    }else{
      cos_d = -10;
    }

    // [cos-theta(tau)] & [tau-daughter's momentum]
    if( 0 < dec_tau && dec_tau < 9 ){
      Gen_hepevt& Dau ( genMgr(Panther_ID(Tau.daFirst())) );
      HepLorentzVector Dau_4V( Dau.PX(), Dau.PY(), Dau.PZ(), Dau.E() );
      Dau_4V.boost( -(Tau_4V.boostVector()) ); // boost to tau-rest frame
      W_4V.boost  ( -(Tau_4V.boostVector()) ); // boost to tau-rest frame
      cos_t          = cos( Dau_4V.vect().angle(W_4V.vect()) ); // [cos-theta(tau)]
      p_daughter_tau = Dau_4V.vect().mag();                     // [p_dau in tau-rest frame ]
      Dau_4V.boost(  (Tau_4V.boostVector()) ); // boost back to lab. frame
      W_4V.boost  (  (Tau_4V.boostVector()) ); // boost back to lab. frame


      Dau_4V.boost( -(W_4V.boostVector()) ); // boost to W-rest frame
      B_4V.boost  ( -(W_4V.boostVector()) ); // boost to W-rest frame
      cos_daughter_w = cos( Dau_4V.vect().angle(B_4V.vect()) ); // [cos-theta(dau) in W-rest frame]
      p_daughter_w   = Dau_4V.vect().mag();                     // [p_dau in W-rest frame ]
      Dau_4V.boost(  (W_4V.boostVector()) ); // boost back to lab. frame
      B_4V.boost  (  (W_4V.boostVector()) ); // boost back to lab. frame


      Dau_4V.boost( -(B_4V.boostVector()) ); // boost to B-rest frame
      p_daughter_b   = Dau_4V.vect().mag();  // [p_dau in B-rest frame ]
      Dau_4V.boost(  (B_4V.boostVector()) ); // boost back to lab. frame

      // added @20160221
      Dau_4V.boost( cmboost ); // boost to cm frame
      p_daughter_cm = Dau_4V.vect().mag();  // [p_dau in cm frame ]
      Dau_4V.boost( cmboost ); // boost back to cm frame

      p_daughter_lab = Dau_4V.vect().mag(); // [p_dau in lab frame ]
